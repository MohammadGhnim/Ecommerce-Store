...
from .serializers import CartSerializers, CartDetailSerializer, OrderDetailSerializer, OrderSerializer
...
class ApplyCoupon(generics.GenericAPIView):
    ...
    def post(self,request,*args, **kwargs):
        user =User.objects.get(username=self.kwargs['username'])
        cart=Cart.objects.get(user=user,completed=False)
        coupon=Coupon.objects.get(id=request.data['coupon_code'])
